      ...
      <tbody>
        <tr v-for="(task, index) in taskStore.tasks" :key="task.id" :class="{'bg-green-100': task.completed}">
          ...
          <td class="border border-gray-200 px-4 py-2 text-center">
            <span class="px-2 py-1 rounded-full text-xs" :class="{'bg-green-500 text-white': task.completed,'bg-yellow-500 text-white': !task.completed}">{{ task.completed ? 'Concluída' : 'Pendente' }}</span>
          </td>
          <td class="border border-gray-200 px-4 py-2 text-center">
...
export default {
  ...
  methods: {
    ...
    confirmAndRemoveTask(id) {
      // Confirmação antes de remover tarefa
    ...
    async fetchSuggestion() {
      // Sugerir tarefa a partir da API
    ...
    removeCompletedTasks() {
      // Remove tarefas concluídas ao recarregar página
